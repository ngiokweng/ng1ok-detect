...
LocalTextSegInfo* save_text_section64(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[])
{
	uint32_t i;
    ...
    char* pwd = "/data/data/ng1ok.detect/libc_so.bin";
	...
	for(i=0; i<eh.e_shnum; i++) {
		if(!strcmp(".text", (sh_str + sh_table[i].sh_name))) {
			LOGE("Found section\t\".text\"\n");
            LOGE("at offset\t0x%08lx\n", sh_table[i].sh_offset);
            LOGE("of size\t\t0x%08lx\n", sh_table[i].sh_size);
	...
	if(!buf) {
        LOGE("Failed to allocate %ldbytes!!\n", sh_table[i].sh_size);
        return NULL;
	}
    ...
    close(fd2);
    close(fd);
    LocalTextSegInfo* localTextSegInfo = (LocalTextSegInfo *)malloc(sizeof(LocalTextSegInfo));

    localTextSegInfo->buf = buf;
    localTextSegInfo->size = sh_table[i].sh_size;
    localTextSegInfo->offset = sh_table[i].sh_offset;

    return localTextSegInfo;
